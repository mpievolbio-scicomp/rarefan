...
public class AlignTwoGenomes_old {
	
	
	HashMap<String,HashMap<Integer,ArrayList<Integer>>> positionMap=new HashMap<String,HashMap<Integer,ArrayList<Integer>>>();
	File alignmentOut;
//	HashMap<String,ArrayList> alignment=new HashMap<String, ArrayList>();
	...
	public static void main(String[] args) {
		...
		AlignTwoGenomes_old atg=new AlignTwoGenomes_old(G1, G2);
	}
	
	public ArrayList<Integer> getPositionQuery(int pos,String ref){
		return positionMap.get(ref).get(pos);
	}
	
	public AlignTwoGenomes_old(File G1,File G2) {
	...
	private void fillMap(ArrayList<Integer> ref,ArrayList<Integer> query,String id) {
		positionMap.put(id,new HashMap<Integer,ArrayList<Integer>>());

		for(int j=0;j<ref.size();j++) {
			if(!positionMap.get(id).containsKey(ref.get(j))) {
				positionMap.get(id).put(ref.get(j),new ArrayList<Integer>());
			}
			positionMap.get(id).get(ref.get(j)).add(query.get(j));
		}
	}
...
//	}
	...
	private HashMap<String, ArrayList> alignGenomes(File G1,File G2) {
		...
		String call="/Applications/Mauve.app/Contents/MacOS/progressiveMauve --collinear --output="+alignmentOut+" "+G1+" "+G2;
